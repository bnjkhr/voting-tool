...
// Das Ausgabeformat lebt in lib/ticket-number.js — dieselbe Funktion, die der
// Generator in beiden Backends nutzt. Nur so testet die Runde "Import hebt den
// Zähler an → Generator gibt die nächste Nummer aus" echten Produktionscode
// statt einer Kopie.
const { formatTicketNumber } = require('./ticket-number');

// Das Prefix muss mindestens alles akzeptieren, was der Generator selbst
// ausgeben kann — sonst liesse sich eine Nummer nicht zurueckimportieren, die
// dasselbe Board vorher vergeben hat. buildTicketPrefix liefert zwar nur
// Buchstaben, aber apps.ticket_prefix kann (Altbestand, Direktimport) auch
// Ziffern enthalten; formatTicketNumber haengt sie ungeprueft an.
const TICKET_NUMBER_PATTERN = /^([A-Za-z][A-Za-z0-9]{0,9})-(\d{1,6})$/;

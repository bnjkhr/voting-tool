...
function buildSuggestionFromRequest(body, appId, userFingerprint, tenantId = LEGACY_TENANT_ID) {
  ...
  const suggestion = {
    ...
    labels: [],
    userFingerprint,
    // Backend-abhaengig: Postgres bekommt ein Date, Firestore den Sentinel. Der
    // Sentinel ist ein opakes Objekt und wuerde in timestamptz als "{}" landen.
...
// `tenant` stammt in beiden Aufrufpfaden aus der Plan-Quelle (Admin-Konsole:
// findActiveTenantBySlug, API-Schluessel: findActiveTenantById) und traegt unter
// Postgres damit `plan`. requiresProUpgradeResolved kapselt die Fail-open-Regel:
// ein nicht aufloesbarer Plan sperrt nicht.
async function createTenantBoard(tenant, body = {}) {
  ...
  return async (req, res, next) => {
    try {
      ...
      req.apiAuth = {
        ...
        // findActiveTenantById laedt unter Postgres aus der Plan-Quelle — dieser
        // Tenant traegt also `plan` und taugt direkt fuer nachgelagerte
        // Pro-Gates (das Free-Plan-Board-Limit in createTenantBoard).
        tenant,
...
//
// Beide Backends: Postgres über eine SQL-Transaktion im Repository, Firestore
// über eine runTransaction. Die Invariante ist identisch — importierte Nummer
// eindeutig UND Zähler darüber hinaus geschoben.
async function createSuggestionWithImportedTicketNumber(tenantApp, tenantId, suggestion, importData) {
  if (usePostgres()) {
    const { conflict, row } = await repos.suggestions.createWithImportedTicketNumber({
      appId: tenantApp.id,
      tenantId,
      ticketNumber: importData.ticketNumber,
      ticketNumberValue: importData.ticketNumberValue,
      data: { id: crypto.randomUUID(), tenantId, ...suggestion },
    });
    if (conflict) {
      return {
        status: 409,
    ...
    await persistScreenshotAttachments(tenantId, 'suggestion', row.id, suggestion.screenshots);
...
app.post('/api/v1/apps', requireApiKey(['boards:write']), rateLimitByApiKey(60000, 30), async (req, res) => {
  try {
    const { status, body } = await createTenantBoard(req.apiAuth.tenant, req.body || {});
...
app.post('/api/v1/apps/:appSlug/suggestions', requireApiKey(['suggestions:write']), rateLimitByApiKey(60000, 30), async (req, res) => {
  try {
    ...
    if (importData?.votes !== undefined) {
      // Nur der Zähler — bewusst ohne votes-Einträge, damit die
      // Doppelabstimmungs-Sperre (die über die votes-Tabelle arbeitet)
      // nicht mit Phantom-Fingerprints gefüttert wird.
      suggestion.votes = importData.votes;
    }
    if (importData?.createdAt) {
      // Postgres nimmt das Date direkt, Firestore einen Timestamp.
      suggestion.createdAt = usePostgres()
        ? importData.createdAt
        : admin.firestore.Timestamp.fromDate(importData.createdAt);
    }
    ...
    if (importData?.ticketNumber) {
      ...
      suggestionId = await createSuggestionRecord(req.apiAuth.tenantId, suggestion);
    }
